...
**Version:** 1.2 — September 16, 2026 (HRBP routing stubbed with a placeholder email)
...
| Submit JD Update Request | Flow | Diff, populate template, save, email HRBP and requester | New |
| JD Templates / JD Change Requests | SharePoint libraries | Template storage / generated requests (HR access only) | New |
| HRBP routing | Flow scope (placeholder) | Returns HRBP email/name; currently a fixed test address | Stubbed — real logic later (D1) |
...
flowchart TD
  ...
  P -- Confirm --> Q[Submit JD Update flow]
  Q --> R[Doc saved, HRBP emailed, copy emailed to user]
```
...
2. **HR-only fields never enter the conversation.** Job Code, Grade, and Status are never returned to the agent as readable content, so neither the orchestrator nor the prompt can repeat them. Job Code is used only as an internal ID; Grade and Status are fetched server-side by the submit flow.
...
4. **Deterministic gate before email.** Nothing is sent until the user explicitly confirms, enforced by a condition node, not by the orchestrator.
...
| `env_HRBP_DefaultEmail` | Text | Your email (stand-in for the HRBP during build) |
| `env_HRBP_DefaultName` | Text | `HR Business Partner` |
| `env_HR_FallbackEmail` | Text | Shared HR mailbox address |

3. Create **connection references** for: SharePoint, Office 365 Outlook, Excel Online (Business) (Office Script), Word Online (Business), and Microsoft Dataverse / AI Builder (Run a prompt). Add Office 365 Users later only if HRBP routing ends up using the requester's profile.
...
| Grade | `Grade` | **No** | No | HRBP email only | ? |
...
| Status | `Status` | **No** | No | Search filter + HRBP email only | Choice? |
...
|---|---|---|
| D1 | How is the HRBP determined? | **Deferred.** For now every request goes to `env_HRBP_DefaultEmail` (your email). The submit flow isolates routing in one scope so real logic can be plugged in later without touching anything else. |
| D2 | How does the user get the document? | **Email attachment only.** The requests library stores Job Code and Grade as metadata, so restrict it to HR. |
...
The template contains **no** Job Code, Grade, or Status, because the requester receives a copy.

### 6.1 Layout

1. **Header:** logo, "Job Description — Change Request"
2. **Request details table:** `RequestId`, `RequesterName`, `SubmittedOn`, `HRBPName`
...
## Phase 7 — Flow: Submit JD Update Request
...
Wrap steps 1–13 in **Scope: Try**; add **Scope: Catch** running after Try *has failed / timed out*.
...
3. **Get HR-only details** — Get items, Job Descriptions list, filter on Job Code = `JobCode`, Top 1
   - **Compose `Grade`**, **Compose `Status`** from the first item (normalize Choice values)
...
5. **Select `AllFields`** — From `outputs('FieldKeys')`
...
6. **Filter array `ChangedFields`** — advanced mode:
   `@not(equals(trim(item()?['ChangeBefore']), trim(item()?['ChangeAfter'])))`

7. **Condition** `length(body('ChangedFields'))` = `0` → Respond `Success = false`, `Message = "No differences found"` → Terminate (Succeeded)

   *This is the deterministic diff: only fields that actually changed get highlighted.*

8. **Scope: `Determine HRBP`** — placeholder routing (D1)
   - **Compose `HRBPEmail`** — `parameters('env_HRBP_DefaultEmail')`
   - **Compose `HRBPName`** — `parameters('env_HRBP_DefaultName')`

   Everything downstream references only `outputs('HRBPEmail')` and `outputs('HRBPName')`. When routing is decided, replace what's **inside** this scope (for example, a lookup that sets these values) and keep those two Compose action names exactly the same, so no later step needs editing. Renaming them would break every reference.

   Add a note on the scope (**... → Add a note**): *"Placeholder: sends all requests to env_HRBP_DefaultEmail until HRBP routing is defined."*

9. **Populate a Microsoft Word template**
   - Header: `RequestId`, `RequesterName`, `SubmittedOn`, `HRBPName`, `ReviewFlags`, `ChangeNotes`
...
10. **Compose `FileName`** — job title with unsafe characters removed (titles may contain `/`):
    `concat('JD Change Request - ', replace(replace(replace(string(outputs('Original')?['JobTitle']), '/', '-'), ':', '-'), '"', ''), ' - ', outputs('RequestId'), '.docx')`

11. **Create file** — JD Change Requests library, name `outputs('FileName')`, content = Word template body → **Update file properties**: RequestId, JobCode, Grade, Requester, HRBP, Status = `Submitted`. Keep **Link to item** for HR.

12. **Send an email (V2) — to HRBP**
    - To: `outputs('HRBPEmail')` (no CC)
    - Subject: `JD change request – @{outputs('Original')?['JobTitle']} – @{outputs('RequestId')}`
    - Body: requester; **HR-only details** (Job Code, Grade, current Status); changed fields `join(body('Select_ChangedLabels'), ', ')`; review flags; link to the file in the library
    - Attachment: file name + Word template body

13. **Send an email (V2) — to requester** (separate email, never a CC of the HRBP email)
    - To: `RequesterEmail`
    - Subject: `Your job description change request – @{outputs('Original')?['JobTitle']}`
    - Body: request ID, HRBP name, changed fields, "the job description won't change until HR approves it." **No Job Code, Grade, Status, or library link.**
    - Attachment: same file

14. **Respond to the agent** (inside Try):

| Output | Value |
|---|---|
| `Success` | `true` |
| `RequestId` | `outputs('RequestId')` |
| `HRBPName` | `outputs('HRBPName')` |
| `ChangedFieldCount` | `length(body('ChangedFields'))` |
| `Message` | `Submitted` |

No file URL is returned to the agent (D2).

**Catch scope:** failure notification to `env_HR_FallbackEmail` with the run URL → **Respond to the agent** with the **same output names** (`Success = false`, `Message = "Submission failed"`, others blank).

> Every Respond action in an agent-called flow must declare identical outputs.

> Emails go out **before** responding, so "submitted" in chat is always true.

**✅ Checkpoint 7:** Run manually with a real original JSON and an edited copy (change Purpose and People Management). The .docx shows those two shaded with struck-through previous text, everything else normal, two summary rows, no HR-only fields. While testing you'll receive **both** emails, since you're the stand-in HRBP and the requester: the HRBP version has Job Code, Grade, and Status; the requester version does not. Check both.
...
```powerfx
{
  ...
  body: [
    ...
    { type: "TextBlock", text: "Submitting sends a change request document to your HR Business Partner and emails you a copy. The job description itself won't change until HR approves it.", wrap: true, isSubtle: true, spacing: "Medium" }
  ]
}
```
...
**S5 — Call Submit JD Update Request**
...
Outputs → `Topic.varSubmitSuccess`, `Topic.varRequestId`, `Topic.varHRBPName`, `Topic.varSubmitMessage`

**S6 — Result**
- **True:** `Global.varSubmitted = true` → Message: *"Submitted! Request **{Topic.varRequestId}** went to {Topic.varHRBPName}, and I've emailed you a copy of the change request document."* → A1 reset (keep `varSubmitted = true`) → End
- **False:** Message: *"Something went wrong submitting your request, and HR has been notified. Your draft is still saved — you can try again or contact your HR Business Partner."* → **Go to step C6**

**✅ Checkpoint 8:** End-to-end in the test pane produces the document, HRBP email, and requester email. Submitting twice doesn't send a second email.
...
| U14 | Double submit | Gate blocks second send |
| U15 | Open requester email + attachment | No Job Code, Grade, or Status anywhere |
...
- [ ] `env_HRBP_DefaultEmail` set for target (test inbox for UAT/QA; real routing before production)
...
3. How HRBPs are assigned (D1) — deferred; placeholder in place
4. `MatchJobTitle` script input/output signature and score scale
5. Status values that count as searchable
6. Approximate number of active job descriptions
