...
**Version:** 1.5 — September 16, 2026 (Search flow wired to existing MatchJobTitle script)
...
1. **Get items** — Job Descriptions list
   ...
   | List Name | `env_JD_ListName` |
   | Filter Query | `Status eq '@{parameters('env_JD_ActiveStatus')}'` (use Status's internal name; no `/Value`, even for a Choice column) |
   | Order By | Blank (the script sorts by score) |
   | Top Count | Blank (pagination controls how many come back) |
   | Limit Columns by View | A list view named **Agent Search** containing only Title, Job Code, Purpose, Status |
   | Limit Entries to Folder | Blank |
   | Include Nested Items | Leave default |

   - **... → Settings → Pagination: On**, threshold above your total JD count (e.g., 5000). Without it, Get items returns only the first 100 items.
   - Limit Columns by View limits **columns only**; the view's own filters and sorting are ignored.
   - If the list ever exceeds 5,000 items, index the Status column (**List settings → Indexed columns**) so the filter doesn't hit the list view threshold.
2. **Initialize variable** `varMatches` (Array, `[]`) and **Initialize variable** `varMatchMethod` (String, `title`). Variables must be initialized at the top level, before any condition or loop.

3. **Select `Catalog`** — From: `value` from Get items. Map (key/value mode) using the **exact property names `MatchJobTitle` reads from `itemsJson`** (check the script code):

   | Key (example) | Value |
   |---|---|
   | `ID` | `item()?['ID']` |
   | `Title` | `item()?['Title']` |
   | `JobCode` | `item()?['Job_x0020_Code']` |

   If the script was originally built to read raw Get items output, you can skip this Select and pass `string(body('Get_items')?['value'])` directly instead.

4. **Run script** (Excel Online Business) — same Location / Document Library / File as the existing flow, Script `MatchJobTitle`
   - `userInput`: `SearchText` trigger input
   - `itemsJson`: `string(body('Catalog'))`

   No script changes needed: it already takes the list as input and returns the top 5 with id, job title, job code, and score.

5. **Compose `Matches`** — `json(outputs('Run_script')?['body/result'])`
   (If the script wraps results in an object, e.g. `{ "matches": [...] }`, add `?['matches']`.)

6. **Compose `TopScore`** — `if(empty(outputs('Matches')), 0, first(outputs('Matches'))?['score'])`

7. **Compose `ExactMatch`** — true only when the top result is a perfect score **and** no second result is also perfect (replace `1` with your script's perfect-match score):
   ```
   if(less(length(outputs('Matches')), 2), greaterOrEquals(outputs('TopScore'), 1), and(greaterOrEquals(outputs('TopScore'), 1), less(outputs('Matches')[1]?['score'], 1)))
   ```
   The outer `if` matters: it keeps `[1]` from being evaluated when there are fewer than two results.

8. **Condition — good title match?** `outputs('TopScore')` is greater than or equal to `{your threshold}`
   - **Yes branch — Apply to each** over `outputs('Matches')` (leave **Concurrency control off** so order is kept):
     1. **Filter array `FindItem`** — From: `body('Get_items')?['value']` · condition: `item()?['ID']` is equal to `items('Apply_to_each')?['id']`
     2. **Append to array variable** `varMatches`:
        ```json
        {
          "jobCode": "@{items('Apply_to_each')?['jobCode']}",
          "jobTitle": "@{items('Apply_to_each')?['jobTitle']}",
          "purposeSnippet": "@{if(greater(length(coalesce(first(body('FindItem'))?['Purpose'], '')), 90), concat(substring(first(body('FindItem'))?['Purpose'], 0, 90), '…'), coalesce(first(body('FindItem'))?['Purpose'], ''))}"
        }
        ```
        Use the script's actual output key names (e.g., if it returns `JobTitle`, use `['JobTitle']`).
   - **No branch:** description fallback (3.1a) → append its matches to `varMatches` in the same shape · **Set variable** `varMatchMethod` = `description`

9. **Respond to the agent** (after the condition):

| Output | Type | Value |
|---|---|---|
| `MatchCount` | Number | `length(variables('varMatches'))` |
| `ExactMatch` | Boolean | `and(equals(variables('varMatchMethod'), 'title'), outputs('ExactMatch'))` |
| `MatchMethod` | Text | `variables('varMatchMethod')` |
| `MatchesJson` | Text | `string(variables('varMatches'))` — shape below |
...
1. **Get items** — Filter Query: `{JobCodeInternalName} eq '@{triggerBody()?['text']}'` (the JobCode input) · Top Count: `1` · Limit Columns by View: blank, or a view with only the 12 visible fields plus Job Code · other parameters blank/default · pagination off
...
| 1.4 | 2026-09-16 | Phase 3 Get items: replaced nonexistent "Select columns" with Limit Columns by View; full parameter tables; corrected Choice-column filter guidance (no `/Value` in Filter Query) |
| 1.5 | 2026-09-16 | Phase 3.1 rebuilt around existing MatchJobTitle (userInput + itemsJson, returns top 5 with id/title/code/score): Catalog Select, Run script, parse, TopScore/ExactMatch expressions, purpose join via Apply to each, final outputs. No script changes required |
